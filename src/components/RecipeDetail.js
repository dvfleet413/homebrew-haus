...
const Recipe = (props) => {
    ...
    return(
        <div className="beer-card">
            ...
            <ul>
                {props.recipe.grain_ingredients.map(grain => <li key={`${grain.id}-grain`}>{grain.weight} oz {grain.name}</li>)}
            </ul>
            ...
            <ul>
                {props.recipe.malt_ingredients.map(malt => <li key={`${malt.id}-malt`}>{malt.weight} lb {malt.name}</li>)}
                {props.recipe.hop_ingredients.filter(hop => hop.hop_type === "bittering").map(hop => <li key={`${hop.id}-hop`}>{hop.weight} oz {hop.name} (bittering hop)</li>)}
            </ul>
            <p>Add water until the total volume in the brew pot is 2.5 gallons. Boil for 45 minutes, then add:</p>
            <ul>
                {props.recipe.hop_ingredients.filter(hop => hop.hop_type === "flavor").map(hop => <li key={`${hop.id}-hop`}>{hop.weight} oz {hop.name} (flavor hop)</li>)}
                <li>1 tsp Irish Moss</li>
            </ul>
            <p>Boil for 12 minutes then add:</p>
            <ul>
                {props.recipe.hop_ingredients.filter(hop => hop.hop_type === "aroma").map(hop => <li key={`${hop.id}-hop`}>{hop.weight} oz {hop.name} (aroma hop)</li>)}
            </ul>
            ...
            <ul>
                {props.recipe.hop_ingredients.filter(hop => hop.hop_type === "dry").map(hop => <li key={`${hop.id}-hop`}>{hop.weight} oz {hop.name} (dry hop)</li>)}
            </ul>
